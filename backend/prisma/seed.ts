...
async function main() {
  ...
  const request1 = await prisma.movementRequest.create({
    data: {
      ...
      toLocation: 'IT Department - Floor 2',
      purpose: 'Need new laptops for IT team expansion',
      totalAmount: 100000,
      userId: employee.id,
      items: {
        create: [
  ...
  const request2 = await prisma.movementRequest.create({
    data: {
      ...
      priority: 'medium',
      status: 'approved',
      neededBy: new Date('2024-12-15'),
      fromLocation: 'Storage Room',
      toLocation: 'HR Department - Floor 1',
      purpose: 'Monthly office supplies for HR team',
      totalAmount: 15000,
      userId: employee.id,
      items: {
        create: [
